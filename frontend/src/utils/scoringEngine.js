...
// ══════════════════════════════════════════════════════════════════════════════
// ⚠️ 双实现同步清单（2026-09-22 新增，血泪教训）
//
// 本文件与 `backend/app/scoring/engine.py` 是**两套独立实现**，而详情页是
// **「本地优先」**（`StockDetail.tryLoadLocalAll` 成功就不调后端接口）⇒
// **用户日常看到的多数是本文件的输出**。因此任何评分/展示逻辑变更**必须两侧
// 同时改**，且验收应以**本地路径**为准。
//
// 历史事故（都源于此）：
//   ① 分差 68.8（本地）vs 72.6（后端）—— 口径漂移
//   ② 2026-09-22：P1 新增「主力极端流入(散户陷阱降分)」标签在本地路径**静默丢失**
//      （本文件 `extractFactors` 写 `&& labelMap[name]` 要求映射存在，
//       而后端是 `.get(name, name)` 有回退）⇒ 用户报"605058 看不到标签"，
//      查了五层才定位 ⇒ **已修**（补 `|| name`）
//
//   本文件                        ←→  后端对应                       核对状态
//   ────────────────────────────────────────────────────────────────────────
//   scoreFlow5()                  ←→  engine._score_flow5()          ✓ 一致（倒U，已核）
//   内联 nb 高换手钳制              ←→  engine._apply_nb_turnover_cap  ✓ 一致
//                                    （⚠️ 后端有 NB_TURNOVER_CAP_ENABLED 开关，
//                                      前端硬编码 ⇒ 后端关开关时两侧会分叉）
//   extractFactors()              ←→  engine._extract_factors()       ✓ 已修（回退）
//   deriveSignal()                ←→  engine._derive_signal()         ✓ 阈值一致 80/65/45/20/35
//   calcBuyPoint()                ←→  engine._calc_buy_point()        ⚠️ 未逐项核对
//   各 scoreXxx() 子项锚点          ←→  engine._score_xxx()             ⚠️ 未逐项核对
//   scoreCapital 归一化            ←→  engine 的 total_w 归一            ✓ 同口径（120）
//
// 💡 新增/修改任何子项打分或展示映射时：先看这张表，改完两侧，再在**本地路径**
//    上验收（断网/清 pack 会强制走后端，反而测不到本地分支）。
// ══════════════════════════════════════════════════════════════════════════════
...
function extractFactors(dims) {
  // ⚠️⚠️ 见文件头部「双实现同步清单」—— 本函数曾因缺回退（`&& labelMap[name]`）
  //   而在本地评分路径**静默丢弃** P1 新增的「主力极端流入(散户陷阱降分)」标签
  //   （后端 `.get(name, name)` 有回退故正常）⇒ 实测 605058 复现，2026-09-22 已修。
  const ups = []
  const downs = []
