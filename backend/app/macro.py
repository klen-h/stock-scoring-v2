...
_SINA_SYMBOLS = [
    ...
    # ★ 2026-09-25（用户需求）：**美股现货 + 中概** —— 补上"外围"里缺的一块。
    #   ⚠️ 与已有的 `hf_NQ`（纳指**期货**）**语义不同**：期货含预期/可 24h 交易，
    #     现货是"已实现"的收盘结果 ⇒ 两者都要，UI 上必须标注区分（别混为一个"纳指"）。
    #   ⚠️ 中国金龙指数是**中概股的正统口径**（比 KWEB/FXI 等 ETF 更标准）；
    #     注意新浪下 `gb_hxc` 是另一只股票（HXC），**指数必须写 `gb_$hxc`**（实测确认）。
    "gb_dji", "gb_inx", "gb_ixic", "gb_$hxc",
]
...
_PANEL_MAP = {
    ...
    "iron":     ("nf_I0", "nf"),        # 铁矿石主连
    # ★ 2026-09-25（用户需求「中概/美股现货」）—— 4 个新品种，全部用 `gb` 族解析。
    #   ⚠️ 与 `nasdaq`（`hf_NQ` = 纳指**期货**）**语义不同**：
    #     期货 24h 交易、含预期；现货是"已实现的收盘结果" ⇒ **两者都要**，
    #     中文名必须带"现货/期货"后缀以免读者混为一谈（这是本项需求的核心价值）。
    #   ⚠️ 中国金龙指数必须写 `gb_$hxc`（`gb_hxc` 是另一只股票 HXC，实测混淆过）。
    "us_dji":   ("gb_dji", "gb"),       # 道琼斯（现货）
    "us_spx":   ("gb_inx", "gb"),       # 标普500（现货）
    "us_ixic":  ("gb_ixic", "gb"),      # 纳斯达克综合（现货）
    "cn_hxc":   ("gb_$hxc", "gb"),      # 纳斯达克中国金龙指数（中概正统口径）
}
...
_OVERNIGHT_KEYS = [
    ...
    ("gold",   "gold",   "黄金"),
    ("nikkei", "nke",    "日经225"),
    # ★ 2026-09-25：美股现货 + 中概（label 带"现货"以区分上面的纳指**期货**）。
    #   ⚠️ 这几项要等 `macro_history` 记过新键才有基准（今天起的下一交易日 15:03 后）。
    ("us_dji", "us_dji", "道指现货"),
