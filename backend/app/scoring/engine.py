...
def _score_flow5(flow5: float) -> float:
    """主力5日净流入(%) -> 0-100（2026-09-13 倒U版）。

    回测背书：mainforce_factor_backtest + flow5_curve_backtest 显示，
    2026-06-29~08-24 全池截面上，倒U曲线去超额 IC 5日/10日分别为 +0.062/+0.039，
    显著优于原单调递增曲线（-0.126/-0.106）；且 Q5 收益从 0.2%/0.94% 改善为
    1.26%/2.04%。极端流入（>+5）+ 高位（price_pos>0.75）子集 5日去超额收益
    从 -3.99% 收窄到 -0.87%。

    锚点：-10→0 / -6→15 / -3→30 / 0→55 / +2→80 / +5→55 / +10→30 / +20→5。
    温和流入（0~+2）最优，极端流入（>=+5）显著降分，体现"散户陷阱"。
