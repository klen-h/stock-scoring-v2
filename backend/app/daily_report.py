...
def _northbound() -> dict:
    """北向资金当日净流入（收盘后取最终值），失败或**已停止披露**时返回空 dict。

    ★ 2026-09-19：交易所 **2024-05-13 起取消北向盘中/当日净流入披露**，东财接口
      存活但**恒返回 0**（`flash/intraday_alerts.py` 已记载"属死数据"，
      `macro.py` 也已按 `if nb.get("total_net"):` 做真值判断）。
      但日报这里**漏了这一层**：`get_northbound()` 会返回非空 dict ⇒ 走 `if nb:`
      ⇒ 把 0 渲染成「**+0.0亿**」⇒ **看起来像"零流入"，实际是"没有数据"**。
      这不只是难看：`_SYSTEM` 里还写着"北向大幅净流出>5亿时必须指出外资离场风险"，
      而该值**永远不会触发** ⇒ 既误导读者也让 LLM 无据可依。
      ⇒ 这里把「总净流入与序列**全为 0**」判为**不可用**，让上层走"已停止披露"分支。
    """
    try:
        ...
        # ★ 死数据判定：总量为 0 且序列全 0 ⇒ 判为不可用（而非"零流入"）
        if not total and not any((p.get("total_net") or 0) for p in series):
            return {}
...
def build_data_md() -> str:
    ...
    if nb:
        ...
        # ★ 2026-09-19：**用户拍板按「已停止披露」处理**，不再尝试接入（附探测依据）。
        #   实测探测确认：北向净流入在**盘中与盘后都不提供了** ——
        #     · kamt.rtmin / kamt.kline（push2his）：净流入列**恒为 0**
        #     · RPT_MUTUAL_DEAL_HISTORY 的 001/003/005（北向）：
        #       FUND_INFLOW / NET_DEAL_AMT / BUY_AMT / SELL_AMT **全部 null**，只剩 DEAL_AMT（成交额）
        #       （对照组：006/002/004 南向的净买入字段**完整保留** ⇒ 南北向被区别对待）
        #     · RPT_MUTUAL_QUOTA_HISTORY：接口已废（空响应）
        #   ⇒ 只剩「北向成交总额」，而**成交额 ≠ 净流入**（买卖各 500 也是大成交额、却零流入）
        #     ⇒ 信息价值不足，不值得为此接新数据源 ⇒ 明确标注"已停止披露"。
        add("- 北向当日净流入：**已停止披露**"
            "（交易所 2024-05-13 起取消该口径；经实测，盘中与盘后的净流入均不再提供，"
            "仅余成交总额，而成交额不等于净流入）。"
            "外资态度请参考 A50 / 离岸人民币 / 恒生科技（更即时），杠杆态度见 1.3b 两融。")
...
_SYSTEM = (
    ...
    "5. 「北向当日净流入」若标注为**已停止披露**，则不得编造任何北向数字、"
    "也不要据此推断外资动向（交易所 2024-05-13 起取消该口径，实测盘中与盘后均无此值；"
    "外资态度请参考 A50 / 离岸人民币 / 恒生科技）。\n"
