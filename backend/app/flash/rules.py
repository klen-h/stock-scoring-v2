...
EVENT_CLUSTERS = [
    ...
    {"name": "农业粮食", "keywords": [
        "农业", "种业", "种植", "粮食", "粮价", "农产品", "农业法",
        "粮食安全", "秋粮", "转基因", "大豆", "玉米", "小麦", "稻谷",
        "养殖", "畜牧", "生猪", "猪肉",
    ]},
]
...
def latest_completed_trading_day(now=None) -> str:
    """「最近的**已完成**交易日」（YYYY-MM-DD）—— 全项目"数据/信号所属日期"的唯一口径。

    ★ 2026-09-19 固化（当天第 3 次踩「拿自然日当交易日」之后）：
      **自然日只在日批正常时点（20:43 盘后）碰巧等于交易日**；凌晨 / 周末 / 节假日 /
      盘前盘中都会错位，表现为 —— 数据包日期永远对不上（空转 30 分钟）、
      信号被打上"周六"标签、连续上榜跨周末清零、按信号日撮合查不到价格。

    规则：当日 **15:00 后且为交易日** → 今天；否则 → 上一个交易日。
      · **15:00 分界是必须的**：盘前/盘中（如周一 09:00）跑到时当日 K 线还不存在，
        手里的数据其实是上一交易日的 ⇒ 用"今天"会让日期与价格**脱钩**。
        与 `scheduler._latest_trading_day()` / `paper_trading` 口径一致。
      · 跳过周末 + `HOLIDAYS`（含 2026 全年：中秋 9-25~9-27、国庆 10-01~10-07）。

    与 `pack_source._latest_available_pack_day()` 的区别：后者针对**数据包发布**
    （22:00 分界，包 19:00 触发、约 20:43 发布），语义更窄，两者不要互相替换。
    """
    d = now or beijing_now()
    if d.hour * 100 + d.minute < 1500:      # 15:00 前 → 先退回上一日
        d -= timedelta(days=1)
    for _ in range(15):                     # 15 天足以跨过春节/国庆连休
        if is_trading_day(d):
            return d.strftime("%Y-%m-%d")
        d -= timedelta(days=1)
    return beijing_now().strftime("%Y-%m-%d")   # 日历异常时兜底，不阻断
