...
依据（scripts/mainforce_factor_backtest.py，2026-09-16 重跑，6,906 全池截面样本）：

  ★ 2026-09-16：筹码引擎换手率单位修复（chips.py：volume 单位「手」被直接
   除以「股」的 float_shares → 换手率低估 100 倍 → 老筹码几乎不换手、
   筹码指标系统性虚高）。下表为**修复后**重跑结果；修复前口径的数字（括号内）
    因虚高筹码而偏乐观，已作废。

  1. 「高位高获利 × 主力流出」组合 = 卖出/风控信号（方向不变，效力减弱）
     5 日 -1.6pt / 10 日 -3.8pt（n=107，胜率 51.4%/47.7%）
     （修复前：5 日 -4.3pt / 10 日 -7.5pt，n=440 → 命中数与差值双降）
  2. 「低位筹码密集 × 主力净流入」= 正向但很弱
     5 日 +0.24pt / 10 日 +0.69pt（n=1748）
     （修复前：10 日 +1.08pt、胜率 66.9%，n=1086）→ 只做展示标签，v1 不加分
  3. 主力资金流与未来收益呈倒 U：温和净流入最优、极端流入最差
     → 单看"大单流入"选股是散户陷阱（去超额 IC -0.10~-0.13）

  单因子（修复后，持有 5 日去超额 IC）：price_pos -0.211 / cost_bias -0.212 /
  winner_ratio -0.218 仍是最强三因子，方向不变；concentration 修复后 IC 反号
  （+0.04）→ **弱因子，不宜作为阈值条件**。
  另：不依赖筹码的 `phase='distribution'` 依然是最锋利的卖出信号
  （5 日去超额 -10.25 / 10 日 -15.23，n=32）。
