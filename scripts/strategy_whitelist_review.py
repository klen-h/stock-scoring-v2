...
def main():
    ...
    for name, v in order:
        ...
        print(f"  {name:<26}{zh:<8}{v.get('n'):>5}{_fmt(v.get('win_rate'),'%',1):>8}"
              ...
              f"{'✅' if pass_exp else '—':>6} {'✅' if pass_win else '—':>6}")

    print(f"\n=== 两套判据的结果 ===")
    print(f"  当前判据 = {rec.WHITELIST_CRITERION}"
          f"（胜率 ≥{rec.WHITELIST_MIN_WIN_RATE}%，样本 ≥{rec.WHITELIST_MIN_SAMPLES}）"
          f" → 白名单 {out.get('list')}")
    ...
    if not out.get("list"):
        print("  ⚠️ 当前白名单为空 → **战法信号不会推送企微**（功能静默）；"
