...
> ## 🔁 换账号 / 新会话 / 换机器：如何接上上下文（2026-09-25 加）
>
> **本项目的"上下文"就是本目录下的 Markdown 文件，不是聊天记录。**
> 它与 CodeBuddy 账号无关，只跟**项目目录**走；且 `.codebuddy/memory/*.md` 已提交到 git
> ⇒ 换机器 `git clone` 后也在。
>
> **新会话第一句话，照抄这句即可**：
> ```
> 先读 .codebuddy/memory/MEMORY.md 与最近 2 天的 daily（今天/昨天），
> 读完复述当前进展与未完成项，再继续。
> ```
>
> **阅读顺序（从粗到细）**：
> 1. **本文件 `MEMORY.md`** —— 跨会话长期事实：架构、项目约定、踩过的坑、用户偏好（必读）
> 2. **`memory/YYYY-MM-DD.md` 最近 1~2 天** —— 当天做什么、改了什么、验证结果、下一步
> 3. 需要更早细节时按日期往前翻（2026-09-03 起，每日一份）
>
> ⚠️ **不要依赖聊天记录**：会话历史通常与账号绑定，换账号可能看不到。唯一可靠的交接方式
> = **把上下文落在这些文件里**（现有工作流已经这么做：每次实质工作都追加 daily，长期事实进本文件）。
> ⚠️ **换账号 / 换机器前先提交推送**：工作区里未 `git commit` 的改动不在仓库里，
> 换机器 clone 会拿不到（本次会话结束时若有未提交改动，先让 AI 提交推送）。
> ⚠️ 会话级"记忆"工具（CodeBuddy Memory 功能）可能存账号云端 ⇒ **不要当作唯一载体**，
> 项目内文件才是权威。
...
## 数据与缓存
- **轮询接口必须"裁列 + 截断正文"（2026-09-25 实测驱动）**：前端 60s/120s 轮询的列表接口
  是最容易被忽视的 egress 大户 —— 页面**开着**就持续消耗。实测两项：
  `push_log`（`content` 上限 8KB，但列表只用 `firstLine`）46 分钟 **≤1.9MB**；
  `coach_alerts`（`SELECT *` 含 `numbers_json` + 完整 `message`）46 分钟 **≈1.28MB**
  ⇒ 页面开 8 小时合计约 **33MB/天**。
  修法：加 `trunc` 参数（`SUBSTR(col, 1, %s)` —— ⚠️ **SQLite 没有 `LEFT()`**，`SUBSTR` 才是双库安全写法），
  列表显式列并**排除**大字段（`numbers_json` 前端零引用）；`trunc=None` 保留原行为给需要全文的调用方。
  ★ 判据：**凡是"前端定时轮询 + 返回表行"的接口，都要问一句"列表真的需要整行吗"**。
- **本机持久缓存的统一模式（本项目已 4 处：flow / l3 / market-emotion-closes / ind-dist-src）**：
  进程内 → **本机 gzip/json（跨进程、跨重启）** → 才回源；指纹用**单行查询**（`MAX(date)`、
  版本号、或 `pg_stat_user_tables` 写入计数）；⚠️ 指纹取不到 ⇒ **不走缓存直接回源**（正确性优先）；
  ⚠️ 空结果/指纹缺失**不写盘**（避免把"无数据"缓存住）。
- **竞价判读（2026-09-25）**：`_auction_read()` 把竞价数字翻成两句人话（**接力意愿** +
  **全市场开局**）+ 一句"该注意什么"，前端置顶显示（数字在下作支撑）。五档组合：
  **顺风开局** / **两种背离**（接力强+大盘低开＝资金只抱团强势股；接力弱+大盘高开＝指数红个股绿）
  / **退潮开局** / 开局一般。阈值：昨涨停股平均高开 ±1.0%、高开:低开 1.5 / 0.67、全市场平均 ±0.1%。
  ★★ **三条硬边界（写进返回值，别被当预测）**：① 只描述**开盘前强度**，方向由 9:30-10:00 验证；
  ② 阈值是**经验初值、未经回测** ⇒ **纯展示、不进信号与仓位**；③ **只落库事实、判读由规则实时算**
  ⇒ 将来校准阈值时历史数据自动受益（"存事实不存结论"）。
  ⚠️ 数据缺失（休市/行情源未就绪）⇒ 返回 None，前端**不显示**（不假装有判读）。
- **竞价数据落库（2026-09-25，A4 竞价看板 #1）**：`market_emotion_daily` 现有**三组互不覆盖**的字段：
  主字段（盘前**首次**算成的预判）／`close_*`（**收盘实际**，对账用）／
  **`auction_*`（只在 9:15-9:25 窗口写，且窗口内允许覆盖 —— 9:25 定稿值最重要）**。
  【为什么必须落库】竞价数据**只在那 10 分钟存在**（内存行情的 open/amount），不落库永久丢失
  ⇒ ① 没有参照系（看不出"高开 47 家"算强还是弱）；② 永远无法回测"竞价强度 → 当日走势"。
  【两条硬约束】① 窗口外**一律不写** —— 9:25 后的 `amount` 已含连续竞价，写成"竞价额"会失真；
  ② `_emotion_daily_row` 必须**组装整行**（SQLite 分支是 `INSERT OR REPLACE`，缺列会被清空）
  ⇒ 新增字段必须同时进 base 的 old 继承，否则收盘写一次就把竞价数据抹掉（双库行为不一致的经典坑）。
...
- ★★ **一个字段不要回答两个相反的问题（2026-09-25，持仓预案"一棍子打死"的根因）**：
  `position_sizing` 的 `position_pct`/`label`（A 主力根据 / B 筹码拥挤 / C 状态禁买）语义是
  **"买入意愿与资格"**；而 `_position_plan` 拿 `suggested_pct == 0` 直接输出"不持有/择机清"
  ⇒ **"禁止买"被读成"必须卖"**（防御市况 `C✗ 禁买` ⇒ 所有持仓判死；工行 `B✗ 追高（获利盘 97%）`
  是"别再追"的理由，却推出"择机清"，逻辑相反）。
  ★ 判据：**凡给用户"该做什么"的建议，先看这个数值原本是为哪个决策算的**；买入档位低 ≠ 持仓危险。
  卖出/减仓的依据应来自**持仓视角**（止损 alerts、浮亏、破位），不是买入闸门。
  ★ 已实施（`position_sizing` 下传 A/B/C 结构化条件 → `trader_brief._position_plan` 只说"不加仓"）；
  配套教训：**"缺证据" ≠ "有反证"** —— `A✗`（未识别吸筹区，罕见信号，多数票都缺）属**无证据**，
  应中性；`B✗`（筹码拥挤＝已大涨）才是**反证**，才配红色警示。混为一谈＝换一种方式"一棍子打死"。
- ★★ **永远不要靠 dict 的字段顺序取值（2026-09-25，涨停清单"数据异常"实为渲染错位）**：
  前端曾用 `Object.values(s)[0]` / `.slice(1,4)` 渲染 zzshare `uplimit_stocks`，
  而该行有 **27 个字段**、**字典序**第一个是 `amount` ⇒ 8 行全显示成 `3.83 · ·`（成交额+空串）。
  ★ 症状伪装成"**数据坏了**"（用户就是这么问的），排查方向会被整个带偏。
  ★ 判据：**跨系统数据一律按字段名取**（本项目已有多次同类教训：LLM 返回、zzshare 行、
  第三方 API）；数据源加字段是常态，位置取值必然在某次上游变更后静默错位。
- ★★ **"完成时刻不确定"的任务：事件驱动 > 服务端轮询 > 客户端轮询（2026-09-26，日批完成通知）**：
  演进三层（本项目实测，用户逐层否决）：① **客户端轮询**（页面定时重试）要求消费方**一直在线**；
  ② **服务端轮询**（后端每 N 分钟查库判断齐了再推）—— 客户端零负担，但仍非实时、有轮询成本；
  ③ **事件驱动回调**（完成方跑完主动打服务端接口 ⇒ 即推）= **用户最终选定**：
  通知时刻 = 完成时刻、零轮询。
  ⚠️ 实现要点：① 回调接口**必须鉴权**（共享 token；**env 未配置时一律 403** = 安全默认）；
  ② 回调**仍要校验产物齐备**（本项目三源：日报+评分快照+矛盾扫描 —— 只看日报会**提前误报**，
  因后端自己另有一条 19:30 起的日报循环；且 `workflow_dispatch` 可能只跑局部任务）；
  ③ 幂等按**业务日**记账（`date_str=day`，跨午夜兜底日也只发一条）；
  ④ 推送走**单点入口**（才能进"今日系统时间线"）；⑤ **失败路径需另配渠道** ——
  事件驱动下"完成方挂了就没有回调"，所以不能用它覆盖失败（本项目由日批内部失败告警承担）。
- ★★ **日批是异步的，别用"固定时间节点"承诺"打开就有"（2026-09-26，复盘 19:30 vs 22:00）**：
  本项目全链 = 上游包（K线+主力，~1h43m ⇒ 20:43 完成）→ `workflow_run` 接棒日批（~1h14m）
  ⇒ **~21:57** 完成、日报实测 **22:16**；⚠️ **兜底日**（第三方挂，22:00 补包）会拖到 **~00:57**。
  ⇒ 时间轴节点只是"提示"，**任何固定点都不保证**；正确做法是**自证（就绪条）+ 自等**
  （未就绪时定时重试、就绪即停）。⚠️ 也不能靠"提前日批"解决：上游必须等**当日 K 线就绪**
  （腾讯收盘后才更新，过早会拿到上一交易日数据 —— 本项目踩过这个坑）。
- ★★ **"某时点能拿到吗"必须按数据来源分类回答（2026-09-26，盘后可靠性审计）**：
  ① **数据库类**（执行记录、推送日志）—— 一定能拿到；
  ② **内存行情类**（组合结算、盘面定稿）—— 交易时段调度器每 2~3 分钟刷全量行情，
     收盘后缓存不再变化 = **收盘定稿** ✓；⚠️ 但**进程在收盘前重启**的 ≤3 分钟窗口内，
     缓存来自**数据库收盘快照（上一交易日）** ⇒ 必须看 `from_snapshot`；
  ③ **日批快照类**（观察池、zzshare、评分快照）—— 由**晚间日批**写入 ⇒ 15:00 拿到的是
     **上一交易日**的（最容易误读成"今天"）⇒ 必须显示 `data_date` / `stale`；
  ④ **外部接口类**（东财板块、腾讯行情）—— 可能限流/失败 ⇒ **不保证**，失败要能静默降级。
  ★★ 纪律：**"请求时刻" ≠ "数据时刻"** —— 一律使用数据自带时刻（`_cache['data_ts']` / `as_of` /
  `data_date`），并让**页面自证**（逐项就绪状态 + 时点），而不是让人对着可能是旧数据的数字做决策。
- ★★ **"今天"与"近 N 天"是两个口径（2026-09-25，今日执行回看）**：窗口统计的条件是
  `date >= 今天-N` ⇒ `days=1` 实际是"**近 1 天**"（**含昨天**），**不能顶替"今天"**。
  ⇒ 凡"今日 XX"必须**按日精确**（`date = 今天`）；接口应显式提供按日参数
  （本项目 `/coach/consistency` 本次加 `day`），并在返回里标识口径（`day` 与 `window_days` 互斥）
  —— 否则消费方无从判断拿到的是哪一套。与"日批日期必须由调用方传入"同源：
  **时间的边界必须显式，不能靠默认值近似**。
- ★★ **用户说"某配置没生效"时先问三件事（2026-09-25，主力阶段配色）**：
  ① **类名生成了吗** —— 在 `dist/assets/*.css` 里 grep 该类名（Tailwind 漏扫 `.js` 会静默丢样式）；
  ② **字段下传了吗** —— 用真实数据探针看后端到底给了什么（本例 `positions_scan` 只给中文
  `phase_cn`、**没给英文枚举** ⇒ 前端无法查配色表，是**结构性缺失**而非样式问题）；
  ③ **是不是"生效了但看不出来"** —— 弱色/边界值最容易误判：本例两只持仓都是**盘整**，
  而"盘整"的设计色就是**淡**（`bg-white/5 text-muted`）⇒ 像没生效。
  ★ 顺序：先排除 ①②（可证伪的技术原因），再谈 ③（设计问题 ⇒ 该问用户要不要调，而不是自己改）。
- ★★ **同一语义的展示元数据必须单点定义（2026-09-25，观察池配色/战法名统一）**：主力阶段配色、
  战法中文名、闸门就绪配色原先都定义在 `ScoreRank.vue` 内部 ⇒ 别的页面要用只能复制 ⇒
  **两份必然漂移**（"要求两处一致"时才发现）。⇒ 抽到 `composables/displayMeta.js` 共享。
  ⚠️ 两个配套前提：① `tailwind.config.js` 的 `content` 必须包含该文件类型
  （本项目 `./src/**/*.{vue,js,ts}`）—— 否则抽到 `.js` 的类名**漏扫 = 样式静默消失且构建不报错**；
  ② 导入时**重命名避开同名符号**（`Workbench.vue` 已有同名 `PHASE_STYLE` 表示**时段**配色，
  直接 import 会造成"配色变了却看不出原因"）。
- ★★ **打桩时不要复制被测代码（2026-09-25，组合盈亏口径验证）**：把组件里的算式复制进测试脚本，
  首跑就**漏抄了一行分支**（`if (!all.length) return null`）⇒ 测的是"复制品"而非原件 ——
  **复制品通过 ≠ 原件正确**。★ 首选抽取**共享函数**；只能复制时必须**逐行对照**。
  ★ 同一轮还犯过"**期望值自己算错**"（把"今日市值"当成"昨收市值"）⇒
  **FAIL 先查自己的算式与测试数据，再怀疑代码**（本轮 3 个 FAIL 全在测试侧，代码是对的）。
- ★ **改布局时"选切点"优于"搬内容"（2026-09-25，盘中分栏）**：给已有页面加左右分栏，
  优先在**现有卡片顺序里找一个语义切点**、只插入容器标签（零搬动 ⇒ 只需保证标签配对），
  而不是把卡片搬进新容器（搬动易漏闭合标签、丢嵌套、错移原有注释归属）。
  竞价看板（左=整体冷热 / 右=具体标的）与盘中（左=盘面 / 右=板块与行动）都用这个办法。
- ★★ **"更直观"要连"高度预算"一起算（2026-09-25，竞价看板图表化）**：把文字标签榜换成
  条形榜（每榜 10 行）后卡片从 ~450px 涨到 ~690px，用户当场反馈"页面长度变长了" ⇒
  **单模块表达力提升可能以整页高度为代价**；加图/加行前先估算目标容器的总高，
  并优先用**同性质信息左右并排**消化增量（本项目已验证有效的省高手段；
  窄屏回退单列、信息不丢）。
- ★★ **本项目的图表偏好（2026-09-25 用户选定）**：榜单/分布类信息优先**纯 CSS 条**
  （零依赖、与卡片风格一致、免 resize 适配），**不把 echarts 引进 Workbench**（高频页，+350KB gzip；
  echarts 只在个股页/板块页/数据中心用）；分布条用"**最大档归一化条长 + 真实家数/占比数字**"；
  刻度用**固定量程**（跨日可比）+ 超界时 clamp 游标但**数值保持真实**（图可失真，读数不可）。
- ★★ **"测试全绿" ≠ "验到了"（2026-09-25，竞价分档的端到端验证）**：脚本进程**不跑 lifespan**
  ⇒ 内存行情缓存为空 ⇒ 接口返回空数据，而**一致性类断言全部 PASS**（`0 == 0`、
  「分档合计 == 家数」在 0==0 时同样为真）⇒ 空数据能让这类断言一路绿灯，**测的是"什么都没发生"**。
  ★ 判据：**做数据类验证时，断言必须先校验"样本非空/非零"**，并显式把数据源准备好
  （如 `restore_market_snapshot(True)` 恢复收盘快照）；"接口真正执行过了吗"要能自证。
- ★★ **审计生产库前先用 `information_schema.columns` 取真实列名，不要猜字段名（2026-09-26，
  复盘完整性审计）**：猜字段名（`content` / `status`）会让查询**直接报错**，而"报错"极易被顺手
  当成"这张表没数据" ⇒ **把基础设施错误误报为业务缺失**，用户会据此去"修一个根本没坏的东西"。
  ⇒ 与"搜不到/查不到 ≠ 不存在"同源：**先用元数据自证"我查的对象是对的"，再谈"全不全"**。
- ★★ **判断"某天数据缺失"前，先确认那天是不是交易日（2026-09-26，一次误报的教训）**：
  实测 `ranking_history` 缺 09-25 就断言"`score_snapshot` 任务失败"，用户一句"25 号不是休盘吗"
  即推翻 —— 2026-09-25 是**中秋休市**（`rules.is_trading_day` = False、`latest_completed_trading_day()`
  = 09-24）⇒ **缺那天才是对的**。⇒ **"某天没数据" ≠ "任务失败"**：先跑交易日判定，
  再谈齐不齐（与"搜不到 ≠ 不存在"同源：**先自证前提，再下结论**）。
  ★★ 附带发现（更要紧）：本项目**日期键有两派** ——
  · **运行日派**：`daily_reports.date`、`contradictions.date`（`run_daily_report()` 不传日期、
    内部 `_today()` = `beijing_now()`；`task_contradiction_scan` 同款）⇒ **休市日也写"当日"记录**；
  · **交易日派**：`ranking_history.rank_date`（休市日不写）；正确范式是 `_batch_trading_day()`
    （`scripts/daily_batch.py`，已接入 zz_finance / weekly_report / subfactor_ic / trader_brief）。
  ⇒ **任何"按某个日期键判齐没齐"的逻辑都会在跨午夜/休市日失效**（本项目通知判据就踩了：
  混合两派 ⇒ 该发的日子永不发）⇒ **判"任务完成"要看写入时间（`created_at`），不要看日期键**。
  ✅ **2026-09-26 已落地**：通知判据改为「三表 `MAX(created_at)` 落在最近 6h 内」；
  `task_contradiction_scan` / `run_daily_report` 的日期键统一到 `_batch_trading_day()`；
  `_save_emotion_daily` 补 `_ensure_emotion_table()`。⚠️ 解析写入时间要**同时兼容** TEXT 北京 ISO
  与 PG timestamptz（实测同一库里两种都有）⇒ 见 `system._to_dt`（含 `+00`→`+00:00` 补丁）。
- ★★ **库里的历史异常 ≠ 现在还有 bug（2026-09-26，一天内第二次误报险情）**：看到 `push_log`
  里 09-25（中秋休市）有 55 条「盘中风险警示」就想报 bug，读源码才发现 `_trading_session()`
  **当天已补 `is_trading_day`**（注释里连用户实测那条「创业板指 -2.68%」都记了）。
  ⇒ 报"数据异常"前必须回答：**它是什么时候产生的？现在的代码还会不会产生？**
  （与"某天没数据 ≠ 任务失败"同源：**先证当下，别拿历史快照下结论**。）
  ★ 附带纪律：**删函数要顺手 grep 全库引用（含注释）** —— 本轮就发现 `Workbench.vue` 里
  还留着指向已删 `report_ready_notify_loop` 的注释。
- ★★ **同一文件的多处 edit 必须串行 + grep 验证引用数（2026-09-26，一次静默失效）**：
  把同一文件的 3 个 edit **同批并行**发出 ⇒ 其中一个**报告成功却完全没落地**
  （文件里存在**结构重复的片段**时最易发生）。⚠️ 这类失败**不报错**、**构建也发现不了**
  （如 Vue 不校验事件绑定/函数是否存在）⇒ 判据只有一个：**改完立刻 grep 那个新符号的出现次数**
  （本次是 `@click.prevent="auctionRowClick"` 应为 2 处、实际只剩 1 处）。
  ⇒ 与"测试全绿 ≠ 验到了"同源：**"工具说成功" ≠ "改动已生效"**。
  ⇒ 纪律：同文件多改 = **一次一个 + 逐个验证**；跨文件才适合并行。
...
- ★★ **给"面板型数据"加品种，必须同步所有消费点（2026-09-25，`macro` 加 4 个外盘品种实测）**：
  本次同步了 **4 处**：`_SINA_SYMBOLS`（请求列表）/ `_PANEL_MAP`（解析映射）/
  **`store.append_macro_history`（历史落库白名单）**/ `_OVERNIGHT_KEYS`（隔夜展示白名单）。
  漏掉**历史白名单**时：面板有数据、但"隔夜变化"因取不到基准**静默跳过**（页面照常渲染只少一行）。
  ★ 与"改枚举数同步 grid 列数"同根：**数据结构加成员 ⇒ 全局搜它的所有消费点**；
  尤其警惕"手工白名单"式的写入方（它不会自动带上新成员）。
- ★★ **接口的 reportName / 代码不能凭记忆猜，多候选一次实测（2026-09-25）**：
  东财减持/质押接口 6 个候选名实测 **2 中 4 挂**；新浪美股代码 15 个实测 14 中 1 挂，
  且**中国金龙指数必须写 `gb_$hxc`**（`gb_hxc` 是另一只股票）。
  ⇒ 与"字段存在 ≠ 有数据"同款：**先实测再设计**，探针成本远低于接错后的返工。
- ★★ **"都叫东财"不代表同一条链路会一起挂（2026-09-25）**：同一天里 `push2/clist`
  被封（`RemoteDisconnected`，板块接口全挂），而 **`datacenter-web` 的报表接口
  （`RPT_LIFT_STAGE` 限售解禁）HTTP 200 完全正常**。
  ⇒ **按"域名 + 端点"判断可用性，不按厂商名**：一个源失败时别顺带放弃同厂商的其他域，
  反之也别以为会一起恢复（本项目 elist/push2 与 datacenter 就是两条独立链路）。
- ★★ **闸门/信号类阈值必须先把"事件研究"跑出来（2026-09-25）**：
  解禁闸门的 `10%` 阈值来自一次实测（1778 条解禁记录 → 268 条可评估样本）：
  ≥10% 档前 20 日**超额 -2.74%、胜率 40%、中位 -3.43%（三项同向为负）**，而 <5% 档反而 **+1.85%**
  ⇒ 阈值有据 ✓。**无回测的阈值等于拍脑袋**（这正是本项目"先量后做"的又一次应用）。
  ⚠️ 同时必须在注释里**明说样本局限**（本例仅覆盖 15%、有选择偏差、未控其他因子
  ⇒ **是关联不是因果**）—— 否则后人会把它当结论用。
...
- ★★ **node 22.1.0 会让 esbuild 构建崩溃（2026-09-25 实测）**：`npm run build` 报
  `[vite:esbuild-transpile] The service was stopped` + Go panic 栈 —— **环境问题非代码错误**。
  ⇒ 先 `nvm use 20.19.0` 再构建即可（vite 清 dist 偶尔还会被 CodeBuddy 的 safe-delete 守卫拦，
  手动 `Remove-Item dist -Recurse -Force` 后重试）。构建失败先看是不是这两类，别去查代码。
