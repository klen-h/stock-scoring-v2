...
## 事件应对方法论（2026-09-15，用户提出+双方共识）
- **定位**：把自己当「主力的最后一个跟随者」——不预测主力下一步，只跟随主力**已留下数据**的动作；慢一步（盘后才有定稿）= 确定性的代价。
- **节奏**：盘中观察（东财实时，只预警不决策）→ 盘后确认（新浪 `mainflow_history` 定稿 + 连续 2 日趋势）→ 次日执行。主力建仓是慢信号，绝不盘中追。
- **FOMC 已证规律**：落地日 D0「杀跌+主力撤」是常态（阴跌观望 6/6 全跌 + 主力 4/4 流出），机会在 D+1 主力回流后、D+3~D+20 修复；主力分步建仓、非一次性。
- **日期口径铁律**：事件分析显式定义 D0=A股反应日（美东决议日+1 的北京交易日）；曾因标签偏移把结论说反，务必写清 D-1/D0/D+1。
- 详见 `PLAN_EVENT_PLAYBOOK_20260915.md`。
