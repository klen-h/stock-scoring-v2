...
- **Coach API（W1 补漏）**：`/api/coach/alerts`（历史）、`POST /alerts/{id}/execute`（回写，放弃必填理由否则 400）、`/consistency`（执行一致性）、`/abandon-reasons`。**收盘回写**：`daily_close_job`（全量含 heavy 求值落库 + T+5 结果回填）挂 `coach_loop` 盘后分支（`coach_daily_close` 幂等）。
