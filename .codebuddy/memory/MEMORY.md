...
- ★★ **真实数据可能"恰好为空" ⇒ 打桩不是可选项，而是与真实调用互补（2026-09-25）**：
  竞价功能改完后跑真实调用，返回**全 0** —— 因为当天**休市、行情缓存为空**（`has_live=False`），
  属预期行为，但**逻辑一行都没被验证到**。⇒ 补**打桩测试**（自造 `stocks`/`closes`）才覆盖到
  排序、家数统计、缺数据跳过、累计口径等 12 项。
  ⇒ **验证矩阵**：`py_compile` → 真实 import → **真实调用** → **打桩测试**。
    · 真实调用能发现"环境/数据形态"问题（PG 的 JSON 列、Decimal、空缓存）；
    · 打桩才能覆盖**你想要的每个分支**（真实数据不一定走到）。
  ⇒ 只做其中一种都会漏。
...
- ★★ **模型降级 / 交接约定（用户 2026-09-25 明确要求）**：若当前模型**额度耗尽而任务未完成**，
  用户希望**切到 `glm5.3-flash` 继续开发**，完成后同样走"审查 → 提交推送 → 收工"。
  ⇒ **本记忆库即交接文档**，必须始终保持最新、可直接被下一个模型读懂：
    · `.codebuddy/memory/YYYY-MM-DD.md` —— 当日做了什么、验证证据、**未完成项与下一步**；
    · `.codebuddy/memory/MEMORY.md` —— 长期纪律、项目约定（如"不自行 push"的历史裁定、
      "改枚举同步硬编码"、"跨库 upsert 语义"、"未 import 静默失效"等）。
    **接手的模型应先读这两份再动手**，不要重头试探。
  ⚠️ 诚实边界：我（模型）**无法自主切换模型、也无法关机** —— 那是用户/客户端的动作。
  我能做的是"**把工作做到可交接**"：代码改完 + 真实验证 + 记档 + 提交推送，让接手者零成本继续。
...
- ★★ **PowerShell 里 `git commit -m '...'`：内容里连中文全角引号都不能有（2026-09-25）**：
  ...
  ⇒ **规则：commit message（以及任何 `-m` 内容）里不要出现引号（半角/全角、单/双都不要）**，
  需要强调就用 `【】` 或直接换词；外层用**单引号**包裹。★ 与"`git push` 进度走 stderr
  会在 PowerShell 渲染成假警报"是同一类"看着像 git 的问题、其实是 shell 的问题"。
- ★★ **手动补历史数据要用 `backfill_lagging()` 而不是 `backfill_daily()`（2026-09-25）**：
  `backend/backfill_history.py` 里 `backfill_daily()` 有**每日配额** `DAILY_STOCK_QUOTA`
  ⇒ 补不齐大盘缺口；`backfill_lagging()` 是同一池 + `quota=None` **不限量且幂等**
  （已同步的自动跳过）⇒ 专为"换库/长假期后大面积滞后"设计。实测 562 只 / 8341 行 / 约 27 分钟。
- ★★ **别拿"进度中间态"当"稳态"来定阈值/判据（2026-09-25，我实际犯过）**：
  手动补 `backtest_prices` 到 93% 时看到"仍缺 56 只" ⇒ 断言"数据源长期无返回、属常态缺失"
  ⇒ 把告警阈值从 50 放宽到 100。**跑完后只剩 2 只** —— 那 56 只只是**当轮还没轮到**。
  ⇒ 教训：**"还没跑完 ≠ 永远跑不完"**。定阈值/判据必须等**终态**；
  中途只能说"看起来偏紧，待终态确认"，不能据此改判据（否则会引入"永远触发/永远不触发"的偏差）。
  同类陷阱：进程内去重标记在任务未完成时被写入（见 `_run_sync` 那条）。
- ★★ **跨模块读"进程内存缓存"必须带落库回退（2026-09-25）**：`market_regime.get_regime_cache()`
  只是 `_REGIME_CACHE`（进程内存）⇒ **新进程/当日尚未判定时返回 `{}`**。若界面上一句关键解释
  依赖它，就会在重启后**静默少一行字**（页面照常渲染 ⇒ 最难发现）。
  ⇒ 一律写成「先读内存缓存，空则回退读落库表」。
  ★ 判据：**凡是"进程内缓存 + 有对应历史表"的数据，都要问一句"新进程首次访问会怎样"**。
- ★★ **给前端的文案不能带 Markdown 标记（2026-09-25）**：后端拼的解释字符串若含 `**粗体**`，
  前端 `{{ }}` 插值是**纯文本** ⇒ 星号原样显示。★ 区分两种文案：
  **企微推送文案**（`push_markdown_batched`，可带 md）vs **前端插值文案**（不可带）。
- ★★ **`scheduler._run_sync` 会吞掉所有异常并返回 `None` ⇒ 判断成败必须看返回值（2026-09-25）**：
  它的设计是"循环不能死"（`try: await asyncio.to_thread(...) except: print + return None`）
  ⇒ 调用方写 `try: v = await _run_sync(fn) ... except:` **永远捕不到异常**，
  若不判 `v` 就会**把失败当成功**（本轮差点因此丢掉当天快照：失败被记上"今日已完成"⇒不再重试）。
  ⇒ **凡在循环里判断某步骤是否成功，看返回值 `if v:`，不要靠 try/except。**
  ★ 泛化：**被调方把异常转成返回值时，调用方的 try/except 就成了摆设** —— 用之前先看它怎么处理异常。
...
- ★★ **「缓存被写入的时刻」≠「数据自身的时刻」（2026-09-25，真实踩到）**：
  `tencent._cache["last_update"]` 表达的是**"缓存可用"**——`restore_market_snapshot` 会**故意**
  把它设成 `time.time()`（注释：避免 overview 误判过期）⇒ 休市日恢复 **09-24** 收盘快照时它是
  **09-25 17:48** ⇒ 拿它当"数据截至"显示，就把**昨天的数据标成今天**（谎报新鲜度）。
  ⇒ 修法：**另设一个只表示数据时刻的键**（本项目 `_cache["data_ts"]` + `from_snapshot`：
  实时刷新=抓取时刻、快照恢复=快照 `saved_at`）。
  ★ 判据：**凡要展示"数据截至/新鲜度"，先问"这个时间戳是谁写的、写的是哪个语义"**
  —— 一个时间戳承担两种语义，早晚会有人用错（本例已核查：其余 3 个消费点语义是对的）。
- ★★ **同一个指标在页面上只能有一个口径（2026-09-25）**：新增"全市场等权涨幅"时必须与既有
  `stats.avg_change_pct` **逐字同口径**（含停牌股记 0、不剔除）——若两处算法不同，页面会出现
  **两个不同的"平均涨幅"**，用户会立刻不信任整页。⇒ 新指标上线前**先找有没有同名旧指标**，
  有则复用其口径（并加断言守住）。同源纪律参见"改枚举数必须同步硬编码"。
- ★★ **批处理任务里，日期必须由调用方传入，不能用 `now()`（2026-09-25）**：
  日批语义是"处理**最近一个已完成交易日**"，但它可能**跨午夜**才跑到（pack 超时重跑、人工补跑）
  ⇒ 任务内 `beijing_now()` 已变成次日 ⇒ ① 判断处被跳过（无自愈 ⇒ 该日数据永远补不出来）；
  ② 即使放行，也往以日期为主键的表里写一条**错日期**的记录。
  ⇒ 统一做法：**判据 + 键值都用 `_batch_trading_day()`，并把日期作为参数传进下层函数**。
  本项目已修：`task_zz_finance` / `task_weekly_report` / `task_subfactor_ic` / **`task_trader_brief`**
  （`collect_brief_data` 与 `generate_trader_brief` 均已支持 `date=`）。
  ⚠️ 改这类"只改判断"是不够的——**必须同时让落库日期可传参**，否则错误从"不生成"变成"错日期"（更糟）。
- ★★ **表里的列可能与代码常量不是同一回事（2026-09-25，真实误读）**：
  `paper_account.initial_capital` 表列 = **100000**，而代码常量 `INITIAL_CAPITAL` = **1000000**
  （表列是**遗留、代码根本不读它**）⇒ 我据"本金 10 万 vs 峰值 101 万"一度判定
  "峰值被污染 10 倍、G2 回撤熔断会永久冻结"——**是误读**，实际回撤 3.49%、一切正常。
  ⇒ 判断"账本/配置是否一致"前，**先 grep 谁在用这个字段**，别只看表面数值冲突。
  ★ 泛化：**同一概念同时存在于"表列"和"代码常量"时，先找出哪一个是事实源**。
- ★★ **"事后不可还原"的指标必须在时点当场落库（2026-09-25）**：内存行情只有**当前值**，
  项目又**没有分时数据**（`get_kline` 只有日线）⇒ 像"14:30 那一刻的涨停家数/指数价"这类
  **时点快照，不记就永久丢失**（`market_tail_snapshot` 就是为此而建）。
  ⇒ 做法：挂在**已有的高频循环**的时间窗里（`intraday_alert_loop` 3 分钟一轮 → 14:30 窗口），
  **零新增定时器**；且当天缺基线时返回"**为什么空 + 什么时候会有**"，不留空白。
  ★ 判据：**凡是"要跟未来某个时刻对比"的指标，先问"这个基准值现在不记，以后还拿得到吗"**。
- ★★ **新增 helper 前先 grep 同名（2026-09-25）**：`market.py` 已有 `_num_or_none`（且更完善，
  还处理东财风格的 `'-'`），我又写了一份 ⇒ 两处定义、后者覆盖前者，功能看似正常但埋雷。
  ★ 同类：**用到的标准库要先确认 import** —— 该文件从未 `import json` 而我用了 `json.dumps`，
  `py_compile` **抓不到**（只有 import/真实调用能发现，与之前 `Optional` 那次同型）。
- ★★ **"用当前状态反推历史"的指标必须声明近似性质（2026-09-25）**：
  组合周回撤按"**当前持仓** × 历史收盘价"回算 ⇒ **窗口内加仓会让市值跳升**，看起来像"上涨"
  （实测：用户 09-24 建仓的新股使回撤被算成 **0%**）。
  ⇒ 这类指标**必须在输出里写明口径与失真条件**（本项目写在 `note` 由前端显示），
  不能让用户当精确值使用。★ 同理：**口径要用"数据不足时也能解释"的文案**
  （"需跨越 2 个交易日，下一个交易日盘后即可见"），而不是留一片空白让人猜是否坏了。
- ★★ **放宽"什么时候可以跑"，必须同步检查"跑起来会不会重复产生副作用"（2026-09-25）**：
  `task_trader_brief` 从"非交易日跳过"改为"按本轮交易日跑"⇒ **休市日也会跑**（cron 按星期触发、
  不看节假日）⇒ 若不补收口，**每个休市日都会把上一交易日的简报重推一次企微**。
  ⇒ 加 `cached ⇒ 不推送`（`wechat.push_markdown_batched` **无去重**，只记 signal_bus 不拦截；
  重推通道留给 `--force`）。
  ★ 判据：**凡"放宽条件/提高频率"的改动，先列出该动作的全部副作用，逐个问"现在会不会重复发生"**。
