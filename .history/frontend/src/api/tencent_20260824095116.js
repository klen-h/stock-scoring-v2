...
export async function fetchRealtimeQuotes(codes) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      ...
      // 失败重试一次（超时/限流）：整批丢失会导致评分池大量股票被静默排除，代价高；
      // 若仍因 CORS/网络错误失败，记录但不中断（代理已在环境层配置）
      try {
        await sleep(1000)
        const retryResult = await fetchBatch(batch)
        Object.assign(result, retryResult)
      } catch (retryError) {
        console.warn(`批次 ${i + 1} 重试仍失败:`, retryError.message)
        if (retryError.message.includes('Failed to fetch') || retryError.message.includes('CORS')) {
