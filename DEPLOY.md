...
3. ⚠️ **免费版两个硬伤**：
   ...
   - 文件系统临时：**每次部署 backend/data/ 清零**。
     现状（2026-09-12）：运行数据已**全部迁到数据库**——快讯/诊断/复盘/信号跟踪/
     宏观历史/ETF 收盘/调度状态，以及财经日历（`flash_calendar`）与 LLM 用量基线
     （`llm_usage_daily`）都在 DB 里，部署清零不再影响业务数据。
     历史上用来兜底的「浏览器数据镜像」**已退役**（它保护的 9 个文件全是空壳，
     却每 5 分钟驱动一次"读诊断正文只为计数"的流量，实测 40MB/天）。
     现在只剩两个**可重建**的文件缓存（`backend/kline_cache.json`、`data/pack/*.db`），
     由 `app/data_files.py` 在启动时 + 每 6 小时核对，异常会企微告警，
     也可通过 `GET /api/system/runtime-files` 查看。
